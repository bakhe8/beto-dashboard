# BetoDashboard Index

Quick links for navigating the project on GitHub:

- Docs site (GitHub Pages): https://bakhe8.github.io/beto-dashboard/
- Docs index (in repo): ./docs/index.md
- Project README: ./README.md

Key sections:

- Introduction
  - Getting Started: ./docs/getting-started.md
  - Architecture: ./docs/architecture.md
  - API Fetching: ./docs/api.md
  - Security: ./docs/security.md
  - Contributing: ./docs/contributing.md
  - Deployment: ./docs/deployment.md
  - Code Style: ./docs/code-style.md
  - Testing: ./docs/testing.md
  - Recipes: ./docs/recipes.md
- Components
  - Modal: ./docs/components/modal.md
  - Table: ./docs/components/table.md
  - ThemeSwitcher: ./docs/components/themeswitcher.md
  - Sidebar: ./docs/components/sidebar.md
  - Loader: ./docs/components/loader.md
  - Toast: ./docs/components/toast.md
- Handbook
  - BetoDashboard v2: ./docs/BetoDashboard_v2.md
  - BetoDashboard Handbook: ./docs/BetoDashboard_v1.md
  - Handbook (WIP): ./docs/handbook/BetoDashboard.md
  - Tradeoffs & Scaling Strategy: ./docs/BetoDashboard_Tradeoffs_and_Scaling_Strategy.md

Tip: Use this page’s URL directly: https://github.com/bakhe8/beto-dashboard/blob/main/index.md
